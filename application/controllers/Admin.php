...
//            exit();
            ...
            if ( ! $this->upload->do_upload("how_vid"))
            {
                ...
                if($sheet['err']==true){
                    redirect(CTRL."Admin/admin_cat_view/$cat_id#param");
                }
                else
...
//            exit();
    ...
    public function update_param_bmi($param_id,$cat_id){
        ...
        if(is_uploaded_file($_FILES['how_vid']['tmp_name'])){
            ...
            if ( ! $this->upload->do_upload("how_vid"))
            {
                ...
                $data['video']=$video_name;
                $this->load->Model('AdminModel');
                $sheet=$this->AdminModel->update_param_bmi($data,$param_id,$med);
                redirect(CTRL."admin/admin_cat_view/$cat_id/");
            }
        }
        else{
    ...
    public function report_main(){
        ...
        $this->load->view('admin/report_main',$sheet);
    }
	public function report_main_norms(){
        $data['category']=$_POST['category'];
        $cat_id=$_POST['category'];
        $data['type']=$_POST['type'];
        if($data['type']=='specific'){
            redirect(CTRL."Admin/print_cat/$cat_id");
        }
        else{
            $this->load->Model('AdminModel');
            $sheet=$this->AdminModel->report_main_norms($data);
            $sheet['type']=$data['type'];
            $this->load->view('admin/report_main_norms',$sheet);
        }
    }
	public function report_sports(){
        ...
        $this->load->Model('AdminModel');
        $sheet=$this->AdminModel->report_sports($data);
        $sheet['type']=$data['type'];
        ...
        $this->load->view('admin/report_sports',$sheet);
        
    }
    public function report_main_howto(){
        $this->load->Model('AdminModel');
        $sheet=$this->AdminModel->report_main_howto();
        $this->load->view('admin/report_main_howto',$sheet);
    }
    public function report_main_limit(){
        $data['category']=$_POST['category'];
        $data['type']=$_POST['type'];
        $this->load->Model('AdminModel');
        $sheet=$this->AdminModel->report_main_limit($data);
        $sheet['type']=$data['type'];
        $this->load->view('admin/report_main_limit',$sheet);
    }
    public function print_group($para_group_id){
        $this->load->Model('AdminModel');
        $sheet=$this->AdminModel->print_group($para_group_id);
//        print_r($sheet);
//        exit();
        $this->load->view('admin/print_group',$sheet);
    }
    public function print_group_1($name,$cat_id){
        $this->load->Model('AdminModel');
        $sheet=$this->AdminModel->print_group_1($name,$cat_id);
//        print_r($sheet);
//        exit();
        $this->load->view('admin/print_group_1',$sheet);
    }
    public function print_group_2($cat_id){
        $this->load->Model('AdminModel');
        $sheet=$this->AdminModel->print_group_2($cat_id);
//        print_r($sheet);
//        exit();
        $this->load->view('admin/print_group_1',$sheet);
    }
    public function print_cat($cat_id){
        $this->load->Model('AdminModel');
        $sheet=$this->AdminModel->print_cat($cat_id);
//        print_r($sheet);
//        exit();
        $this->load->view('admin/print_cat',$sheet);
    }
}
?>